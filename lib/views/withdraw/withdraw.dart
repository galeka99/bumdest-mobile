...
class _WithdrawPageState extends State<WithdrawPage> {
  ...
  Future<void> _getMethods() async {
    try {
      dynamic data = await Api.get('/v1/withdraw/methods');
      setState(() {
  ...
  Future<void> _topup() async {
    ...
    try {
      ...
      Helper.toast(context, 'Withdraw success');
      Nav.push(WithdrawHistoryPage());
    } catch (e) {
      if (e is ApiError) {
        Helper.toast(context, e.message, isError: true);
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(
        ...
        actions: [
          IconButton(
            onPressed: () => Nav.push(WithdrawHistoryPage()),
              ...
              child: ListView(
                ...
                children: [
                  ...
                  TextFormField(
                    ...
                    validator: (str) {
                      if (str!.isEmpty) return 'Withdraw amount cannot be empty';
                      if (int.parse(str) < 10000) return 'Minimum withdraw is 10.000';
                      return null;
                    },
                  ),
                  SizedBox(height: 20),
