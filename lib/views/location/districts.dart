...
class DistrictsPage extends StatefulWidget {
  final int cityId;
  const DistrictsPage(this.cityId, {Key? key}) : super(key: key);
...
class _DistrictsPageState extends State<DistrictsPage> {
  ...
  List<DistrictModel> _items = [];
  List<DistrictModel> _temp = [];

  Future<void> _getItems() async {
    try {
      dynamic data = await Api.get('/v1/location/city/${widget.cityId}', auth: false);
      setState(() {
        _items = (data as List).map((e) => DistrictModel.parse(e)).toList();
        _temp = _items;
      });
    } catch (e) {
      if (e is ApiError) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(context, 'Select District'),
      body: Column(
        children: [
          Padding(
