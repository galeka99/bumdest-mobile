...
class CitiesPage extends StatefulWidget {
  ...
  const CitiesPage(this.provinceId, {Key? key}) : super(key: key);
...
class _CitiesPageState extends State<CitiesPage> {
  ...
  Future<void> _getItems() async {
    try {
      dynamic data = await Api.get('/v1/location/province/${widget.provinceId}', auth: false);
      setState(() {
        _items = (data as List).map((e) => CityModel.parse(e)).toList();
        _temp = _items;
      });
    } catch (e) {
      if (e is ApiError) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(context, 'Select City'),
      body: Column(
        children: [
          Padding(
